...
let plistFileName:String = "elist"
...
struct Plist {
  ...
  var destPath:String? {
   // print ("source = \(sourcePath)")
    ...
    var dir = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true)[0]
    //print("dir :: \(dir)")
    (dir as NSString).appendingPathComponent("\(name).plist" ) //why this not exc..
   
    var p = (dir  as NSString)
    //print("dir2 :: \(dir)")
  ...
  func getMutablePlistFile() -> NSMutableDictionary?
  {
    ...
    if fileManager.fileExists(atPath: destPath!)
    {
        guard let dict = NSMutableDictionary(contentsOfFile: destPath!)
        else
        {
            print("mutfail")
            //print(fileManager.fileExists(atPath: destPath!))
            //print(destPath!)
            //print(NSMutableDictionary(contentsOfFile: sourcePath!))
        return NSMutableDictionary(contentsOfFile: destPath!) //<-nil
...
class PlistManager {
  ...
  func saveValue(value:AnyObject, forKey:String) {
    ...
    if let plist = Plist(name: plistFileName) {
      //print("aaaaaaaaa \(plist)")
      let dict = plist.getMutablePlistFile()! //<-error
